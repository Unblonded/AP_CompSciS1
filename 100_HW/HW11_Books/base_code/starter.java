...
class starter {
		...
		*/
		while(true){
			...
			if (choice.equals("check")) {
				System.out.print("What word to check: ");
				String word = sc.nextLine();
				System.out.println("'" + word + "' was found " + checksArray(arr, word) + " times");
			}
			
			if (choice.equals("printNum")) {
				System.out.print("Enter Num: ");
				int num = sc.nextInt();
				sc.nextLine(); //clear buffer thing
				printNumber(arr, num);
			}
	...
	*/
	public static void printNumber(String[] arr, int num) {
        if (num > arr.length) {
            System.out.println("Number Too Big Try Smaller One");
        } 
        else {
            for(int i = 0; i < num; i++) {
                System.out.print(arr[i] + " ");
